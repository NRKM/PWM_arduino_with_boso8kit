...
#define MOTOR_CONTROL   300
#define CUTTING_START   350
#define OKURI_START2    400

volatile int enc_count        = 0;
const int target_count1       = 1200;
const int target_count2       = 3600;
double P                      = 0.02;
double I                      = 0;
double D                      = 0;
double PWM_MAX                = 255;
...
int Okuri_Time1               = 1000;
int Okuri_Time2               = 500;
int Cutting_Time              = 200;
...
void loop() {
  ...
  case MOTOR_START:
                PID_Start();
                Running_Status = MOTOR_CONTROL;
    break;
  case MOTOR_CONTROL:
                Running_Status = MOTOR_CONTROL;
                PID_Control(target_count2);
    break;
  case OKURI_START2:
                Okuri_Start(Okuri_Time2);
                Running_Status = WAIT_INPUT;
...
void Okuri_Start(int OkuriTime){
...
void PID_Control(int target_count){
  static   int i              = 0;
...
void COrigin_Check(int p_deviation){
  #define Origin_Min -100
  #define Origin_Max 100
  #define Origin_Check_Time 5000
  #define Origin_Check_Execution_Time 7000
  static int i=0;
  static int j=0;
  //Counts execution counts of this routine
  j++;

  if(PID_On == 1 && j <= Origin_Check_Execution_Time){
    if(PID_On == 1 && Origin_Min <= p_deviation && p_deviation <= Origin_Max){
      i++;
      if(i >= Origin_Check_Time){
        //if(COrigin_Sensor == HIGH)
          //Running_Status step next stages.
          Running_Status = OKURI_START2;
          i=0;
          Break();
          PID_On = 0;
         //else if(COrigin_Sensor == LOW)
          //it output error signal.
          j = 0;
      }
    }
  }
  else if(PID_On == 1 && Origin_Check_Execution_Time <= j){
          //it output error signal.    
          j = 0;
  }
}
