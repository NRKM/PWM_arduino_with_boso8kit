  ...
  const int M_pinA              = 0; //A相割り込み0
  const int M_pinB              = 1; //B相割り込み1
  const int M_PWM               = 2; //PWM
  const int M_pinFW             = 3; //Foword Trigger
  const int M_pinBR             = 24; //Break Trigger
  const int M_pinRE             = 25; //Reverse Trigger
  //const int M_pinFW             = 2; //Foword Trigger
  //const int M_pinBR             = 3; //Break Trigger
  //const int M_pinRE             = 24; //Reverse Trigger
  //const int M_PWM               = 25; //PWM
  ...
  void setup() {
    ...
    //
    attachInterrupt(M_pinA           , enc_changedPinA, CHANGE);//pinAの信号変化に合わせて割り込み処理
    attachInterrupt(M_pinB           , enc_changedPinB, CHANGE);//pinBの信号変化に合わせて割り込み処理  
  }
  ...
  void Okuri_Start2(){
      ...
      SerialUSB.println("Okuri_Start2_On");
      digitalWrite(T_OkuriMotor, HIGH);
      digitalWrite(T_OkuriMotor_Middle, HIGH);
      delay(300);
      digitalWrite(T_OkuriMotor, LOW);
      digitalWrite(T_OkuriMotor_Middle, LOW);
      SerialUSB.println("Okuri_Start2_Off");
